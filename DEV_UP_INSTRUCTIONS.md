...
**Repository:** `GlacierEQ/adobe-creative-provenance-gate`  
**Company lens (independent):** Adobe (`adobe`)  
**Innovation:** Creative Provenance Gate  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Adobe affiliation, proprietary access, or production deployment.
...
### Brick wall
Scaling automation without flattening human judgment, losing asset lineage, or creating unauthorized transformations.

### Mechanism to implement
Bind each creative export to a content-addressed provenance receipt (source mix, model id, rights claim, transform chain) and refuse silent mutation.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Adobe employment, endorsement, or proprietary systems access.
...
File: `src/creative_provenance_gate.py`

- Expand `CreativeProvenanceGate` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_creative_provenance_gate.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_creative_provenance_gate.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Adobe APIs
